...
%
% Ms --------- Structure with measured feature locations
%
%+------------------------------------------------------------------------------+
...
Se.delt = dtIn;
XMat = []; tVec = [];
Ms.rxArray = cell(1, 10); Ms.RCIArray = cell(1, 10); Ms.rcArray = cell(1, 10);
interval = 50; timeElapsed = interval + 1; numMeas = 0;
...
for kk=1:N-1
  ...
  if(~isempty(E.statek))
    for ii = 1:Nf
        if((ii == 2) && (numMeas <= 10))
            if(timeElapsed > interval && ~isempty(rx))
                numMeas = numMeas + 1;
                Ms.rxArray{numMeas} = rx;
                Ms.RCIArray{numMeas} = P.sensorParams.RCB * E.statek.RBI;
                Ms.rcArray{numMeas} = E.statek.rI + E.statek.RBI'*P.sensorParams.rocB;
                timeElapsed = 0;
            else
